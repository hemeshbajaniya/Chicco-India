    ...
    try {
        if (!empty(viewData) && viewData != undefined && empty(viewData.cart)) {
            for (var j = 0; j < viewData.order.items.totalQuantity; j++) {
                var amount = !empty(viewData.order.totals.subTotal) ? viewData.order.totals.subTotal : '';
                products.push({
                    ...
                    price: Number(amount.replace(/[^0-9.-]+/g,"")),
            ...
            for (var j = 0; j < viewData.quantityTotal; j++) {
                if(viewData.productId == viewData.cart.items[j].id){
                    var amount = !empty(viewData.cart.items[j].price.sales.formatted) ? viewData.cart.items[j].price.sales.formatted : '';
                    products.push({
